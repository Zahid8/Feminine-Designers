...
add column if not exists fabric_price numeric(12,2) not null default 0 check (fabric_price >= 0);

alter table order_items
add column if not exists dye_price numeric(12,2) not null default 0 check (dye_price >= 0);
